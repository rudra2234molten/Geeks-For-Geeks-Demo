...
import Weekend_batch_banner from './Weekend_batch_banner'
...
let bannerArray = [
  ...
  },
  {
    ...
    rating:4.7,
    title: 'JAVA Backend Development - Live Extra',
...
function Weekend_banner_parent() {
  return (
    <>
      <div className="weekend_heading flex">
        <h3>Three 90 Batches Starting this weekend</h3>
        <p>View All <i className="ri-arrow-right-s-fill"></i></p>
      </div>
      <div className="weekend_banner_container flex">
        {bannerArray.map((banners) => (
          <Weekend_batch_banner key={Math.random()} image={banners.image} interested={banners.interested} rating={banners.rating} title={banners.title} level={banners.level} available={banners.available} />
        ))}
      </div>
    </>
  )
...
export default Weekend_banner_parent